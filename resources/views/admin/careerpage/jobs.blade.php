...
<style>
                ...
                <div class="ms-panel">
                    <div class="ms-panel-header">
                        ...
                        <div class="addnewdata">
                            <button type="button" class="btn btn-sm btn-pill btn-dark has-icon clientBtn" title="Add"  data-toggle="modal" data-target="#addJob"><i class="fa fa-plus"></i> Job</button>
                        </div>
                                            ...
                                            @foreach ($job->requirements as $requirement)
                                                @foreach ($job->responsibilities as $responsibility)
                                                    <tr class="item{{ $job->id }}">
                                                        <td class="">{{ \Illuminate\Support\Str::limit($job->title, 20, $end='...')}}
                                                            <a href="{{ route('job.applicant', [$job->id]) }}">
                                                                <span class="total_applicant badge badge-outline-secondary">{{ $job->applicants->count() }}</span>
                                                            </a>
                                                        </td>
                                                        <td class="">{{ $job->location }}</td>
                                                        <td>{!! \Illuminate\Support\Str::limit($job->short_description, 20, $end='...') !!}</td>

                                                        <td class="">{{ strip_tags($job->formated_description) }}...</td>
                                                        <td class="">
                                                            {{ strip_tags($requirement->formated_requirement) }}...</td>

                                                        <td class="">
                                                            {{ strip_tags($responsibility->formated_responsibility) }}...
                                                        </td>
                                                        <td class="actionBtn">
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="headCenter"><h5 class="modal-title mt-0" id="myModalLabel">Add New Job</h5></div>
    ...
    <div class="modal fade" id="edit_modal" tabindex="-1" role="dialog" aria-labelledby="edit_modal">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="headCenter"><h5 class="modal-title mt-0" id="myModalLabel">Update Job</h5></div>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
                <div class="modal-body">
    ...
    <div class="modal fade" id="viewModal" tabindex="-1" role="dialog" aria-labelledby="viewMessageModal">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="headCenter"><h5 class="modal-title mt-0" id="myModalLabel">Job Details</h5></div>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
...
<script src="https://cdn.tiny.cloud/1/hp6iw1obnykngaej8r497s5a4jmcv78ctja0xrne9dpn602o/tinymce/5/tinymce.min.js" referrerpolicy="origin"></script>

    <script type='text/javascript'>

    tinymce.init({
        selector: 'textarea',
        plugins: 'a11ychecker advcode casechange formatpainter linkchecker autolink lists checklist media mediaembed pageembed permanentpen powerpaste table advtable tinycomments tinymcespellchecker paste',
                ...
                success: function(response) {
                    if (response.success == true) {
                        ...
                        var trDOM = jobTable.row.add([
                            "" + response.data.title + "<a href=" + url +"><span class='total_applicant badge badge-outline-secondary'>" + response.data.applicants + "</span></a> ",
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success == true) {
                        ...
                        tinymce.get("edit_requirement").setContent(response.data.requirement);
                        tinymce.get("edit_responsibility").setContent(response.data.responsibility);
